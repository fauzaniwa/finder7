    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      if (artworkSlug) {
        fetch(`get_detailpameran.php?karya=${artworkSlug}`)
          .then(response => {
            if (!response.ok) {
                return response.json().then(errorData => {
                    throw new Error(errorData.message || 'Respons jaringan tidak OK');
                });
            }
          ...
          .then(data => {
            if (data.error) {
                mediaContainer.innerHTML = `<p class='text-center text-red-500 mt-10'>Terjadi kesalahan: ${data.message}</p>`;
                return;
            }

            // Memperbarui konten HTML
            artworkTitle.textContent = data.karya.judul_karya;
            creatorName.textContent = data.karya.nama_karya;
            artworkDescription.textContent = data.karya.deskripsi;
            likeButton.dataset.idkarya = data.karya.id_karya;
            likesCountSpan.textContent = `${data.karya.likes_count} Likes`;

            if (data.karya.user_liked > 0) {
            ...
            if (data.karya.file_type === 'video') {
                mediaContainer.innerHTML = `
                    <video id="artwork-video" controls autoplay loop muted class="w-full h-full object-cover">
                        <source src="${data.karya.pict_url}" type="video/${data.karya.pict_url.split('.').pop()}">
                        Your browser does not support the video tag.
                    </video>`;
            } else {
                mediaContainer.innerHTML = `
                    <img id="artwork-image" src="${data.karya.pict_url}" alt="${data.karya.judul_karya}"
