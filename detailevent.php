...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['id_event']) && isset($_POST['form_type'])) {
  if (!$user_id) {
    // Ini seharusnya tidak terpanggil jika JS bekerja dengan benar, tapi sebagai fallback
  ...
  if (in_array($id_event, $events_with_tickets)) {
    ...
    if (!$upload_error && $proceed_to_insert) {
      ...
      if (mysqli_stmt_execute($stmt_insert_tiket)) {
        // [MODIFIED] Mengganti alert dengan script untuk membuka ThankYouModal
        echo "<script>
...
$query_event_detail = "SELECT * FROM event WHERE id_event = ? AND show_event = 1";
...
<body class="bg-[#0D0D0D]">
  ...
  <section class="w-full min-h-screen pt-32 pb-32 bg-[#0D0D0D] font-work">
    <div class="w-[90%] mx-auto flex flex-col lg:flex-row gap-10 lg:items-center bg-[#131313] p-6 rounded-xl">
      ...
      <div class="order-last lg:order-first w-full lg:w-1/2 space-y-5">
        <p class="text-sm text-white/70">Homepage / Jadwal / <span class="text-yellow-400 italic">Detail
            Acara</span></p>

        <?php if ($row_event['tiket_event'] <= 0): ?>
          <span
            class="inline-block border border-green-400 text-green-400 text-sm font-semibold px-4 py-1 rounded-full">FREE
            ENTRY</span>
        <?php endif; ?>
        ...
        <?php if ($row_event['tiket_event'] > 0): ?>
          <div class="flex items-center gap-2 bg-gray-800/50 text-white font-semibold px-4 py-2 rounded-full w-fit">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-green-400" viewBox="0 0 20 20"
              fill="currentColor">
              <path
                d="M8.433 7.418c.158-.103.346-.196.567-.267v1.698a2.5 2.5 0 00-1.134 0v-1.43zM9.5 0a9.5 9.5 0 100 19 9.5 9.5 0 000-19zM.5 9.5a9 9 0 1118 0 9 9 0 01-18 0z" />
              <path
                d="M9 7.083A2.502 2.502 0 007.5 9.5v.001c0 .895.46 1.708 1.134 2.166a2.5 2.5 0 002.732 0C12.04 11.21 12.5 10.396 12.5 9.501V9.5A2.5 2.5 0 009 7.083z" />
            </svg>
        ...
        <div class="space-y-2 text-white">
          <div class="flex items-center gap-3">
            ...
            <span><?php echo htmlspecialchars(date('d F Y', strtotime($row_event['waktu_event']))); ?> |
              <?php echo htmlspecialchars($row_event['jadwal_event']) ?></span>
          </div>
          <div class="flex items-center gap-3">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round"
        ...
        <?php if ($row_event['tiket_event'] <= 0): ?>
          <p class="text-yellow-400 text-sm italic">*untuk pelajar SMA/SMK diwajibkan mengupload KTP/Kartu Pelajar
          </p>
        <?php endif; ?>
        ...
        <div class="w-full flex py-4">
          <?php if (!$user_id): ?>
            <button id="openNotLoginModalBtn" type="button"
              class="bg-[#00E091] hover:bg-[#00c77e] text-black font-semibold px-8 py-3 rounded-full text-lg transition-all">Daftar</button>
          ...
          <?php else: ?>
            <?php if ($row_event['event_status'] == 1 && $sisa_kuota > 0): ?>
              <button id="openRegistrationModalBtn" type="button"
            ...
            <?php elseif ($row_event['event_status'] == 2): ?>
              <button disabled class="bg-[#202020] text-white px-8 py-3 rounded-full text-lg cursor-not-allowed">Pendaftaran
                Belum Dibuka</button>
            <?php elseif ($row_event['event_status'] == 3): ?>
              <button disabled class="bg-[#202020] text-white px-8 py-3 rounded-full text-lg cursor-not-allowed">Event Sudah
                Berakhir</button>
            <?php else: ?>
              <button disabled class="bg-[#202020] text-white px-8 py-3 rounded-full text-lg cursor-not-allowed">Tiket Telah
                Habis</button>
            <?php endif; ?>
          <?php endif; ?>
        </div>
    ...
    <div id="registrationModal"
      class="fixed inset-0 bg-black bg-opacity-80 z-50 flex items-center justify-center p-4 hidden">
      <?php if ($row_event['tiket_event'] <= 0): // Modal FREE ?>
        <div class="bg-white rounded-2xl p-8 max-w-lg w-full relative">
          <button type="button" id="closeRegistrationModalBtn"
          ...
          <form id="freeRegistrationForm" method="post" action="" enctype="multipart/form-data">
            ...
            <div class="mb-4">
              <label for="nama_lengkap_free" class="block text-sm font-medium text-gray-700">Nama
                Lengkap</label>
            ...
            <div class="mb-6">
              <label for="kartu_pelajar"
                class="w-full flex justify-between items-center px-4 py-3 bg-gray-100 text-gray-500 rounded-lg border-2 border-dashed border-gray-300 cursor-pointer hover:bg-gray-200">
            ...
            <button type="submit"
              class="w-full bg-[#00E091] hover:bg-[#00c77e] text-black font-bold py-3 rounded-lg text-lg">Kirim</button>
          </form>
        </div>
      <?php else: // Modal PAID ?>
          ...
          <form id="paidRegistrationForm" method="post" action="" enctype="multipart/form-data">
            ...
            <div class="flex flex-col md:flex-row gap-8">
              <div class="w-full md:w-1/2 space-y-4">
                <div>
                  <label for="nama_lengkap_paid" class="block text-sm font-medium text-gray-700">Nama
                    Lengkap <span class="text-red-500">*wajib diisi</span></label>
                  <input type="text" id="nama_lengkap_paid" name="nama_lengkap"
                    class="mt-1 block w-full px-3 py-2 bg-gray-100 border-gray-300 rounded-lg focus:outline-none focus:ring-green-500 focus:border-green-500"
                    required>
                </div>
                <div>
                  <label for="email_paid" class="block text-sm font-medium text-gray-700">Email <span
                      class="text-red-500">*wajib diisi</span></label>
                  <input type="email" id="email_paid" name="email"
                    class="mt-1 block w-full px-3 py-2 bg-gray-100 border-gray-300 rounded-lg focus:outline-none focus:ring-green-500 focus:border-green-500"
                    required>
                </div>
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-1">Fee <span class="text-red-500">*wajib
                      upload</span></label>
                  <label for="bukti_pembayaran"
                    class="w-full flex justify-between items-center px-4 py-3 bg-gray-100 text-gray-500 rounded-lg border-2 border-dashed border-gray-300 cursor-pointer hover:bg-gray-200">
                    <span id="file-name-paid">qris-bukti-payment</span>
                    <span class="bg-[#00E091] text-black font-semibold px-4 py-1 rounded-md">Upload
                      File</span>
                  </label>
              ...
              <div class="w-full md:w-1/2">
                <div class="flex justify-between items-center mb-4">
                  ...
                  <span class="bg-gray-800 text-white px-4 py-2 rounded-full font-semibold">Rp.
                    <?php echo number_format($row_event['tiket_event'], 0, ',', '.'); ?>,-</span>
                </div>
            ...
            <button type="submit"
              class="w-full mt-6 bg-[#00E091] hover:bg-[#00c77e] text-black font-bold py-3 rounded-lg text-lg">Kirim</button>
          </form>
        </div>
      <?php endif; ?>
    </div>
    ...
    <div id="notLoginModal"
      ...
      <div class="bg-white rounded-2xl p-8 max-w-md w-full text-center relative">
        <button type="button" id="closeNotLoginModalBtn"
          class="absolute top-4 right-4 text-gray-500 hover:text-black text-2xl font-bold">&times;</button>
        ...
        <a href="register.php"
          class="inline-block bg-[#00E091] hover:bg-[#00c77e] text-black font-semibold px-8 py-3 rounded-full text-lg transition-all">Buat
          Akun</a>
      </div>
    </div>
    ...
    <div id="confirmCloseModal"
      class="fixed inset-0 bg-black bg-opacity-80 z-[60] flex items-center justify-center p-4 hidden">
      <div class="bg-white rounded-2xl p-8 max-w-md w-full text-center relative">
        <button type="button" id="cancelCloseModalBtn"
          class="absolute top-4 right-4 text-gray-500 hover:text-black text-2xl font-bold">&times;</button>
        ...
        <button type="button" id="confirmCloseModalBtn"
          class="inline-block bg-[#00E091] hover:bg-[#00c77e] text-black font-semibold px-8 py-3 rounded-2xl text-lg transition-all">Tutup</button>
      </div>
    </div>
    ...
    <div id="thankYouModal"
      ...
      <div class="bg-white rounded-2xl p-8 max-w-md w-full text-center relative">
        ...
        <p class="text-gray-700 mb-6">Tunggu 1x24 jam dan cek berkala halaman account page pada bagian Tiket.
          Hubungi CP admin dibawah jika dalam batas waktu tersebut tiket belum didapatkan atau terdapat bug
          sistem. CP : 085155471153</p>
        <a href="account.php"
          class="inline-block bg-[#00E091] hover:bg-[#00c77e] text-black font-semibold px-8 py-3 rounded-2xl text-lg transition-all">Tutup</a>
      </div>
    </div>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function () {
      ...
      // Manual closing for thank you modal (if needed, currently redirects)
      if (thankYouModal) {
        ...
        if (thankYouCloseBtn) {
          thankYouCloseBtn.addEventListener('click', () => {
            ...
            // Redirect to account.php as originally intended by PHP
      ...
      if (fileInputFree && fileNameSpanFree) {
        fileInputFree.addEventListener('change', () => {
          fileNameSpanFree.textContent = fileInputFree.files.length > 0 ? fileInputFree.files[0]
            .name : '*opsional KTP/Kartu pelajar untuk pelajar SMA/SMK';
        });
      }
      ...
      if (fileInputPaid && fileNameSpanPaid) {
        fileInputPaid.addEventListener('change', () => {
          fileNameSpanPaid.textContent = fileInputPaid.files.length > 0 ? fileInputPaid.files[0]
            .name : 'qris-bukti-payment';
        });
      }
    });
