...
<head>
  ...
  <title>Dashboard - Data Absen Pameran</title>
...
<section class="section main-section">
  ...
  <div class="flex items-center mb-4">
    ...
    <!-- Sort By Created -->
    <a href="?sort=created_asc" class="button ml-auto">Sort by Created Asc</a>
  ...
  <div class="card has-table">
    <div class="card-content">
      ...
      // Query dasar untuk mengambil data absen dengan join tabel absenpameran dan user
      $query = "SELECT absenpameran.id_absenpameran, absenpameran.kode_absen, absenpameran.created_absen, user.nama
                FROM absenpameran
                INNER JOIN user ON absenpameran.kode_absen = user.kode_account";

      // Filter Tanggal
      if (!empty($_GET['date_filter'])) {
        $date_filter = $_GET['date_filter'];
        $query .= " WHERE DATE(absenpameran.created_absen) = ?";
      }
      ...
      switch ($sort) {
        case 'created_asc':
          $query .= " ORDER BY absenpameran.created_absen ASC";
          break;
        case 'created_desc':
        default:
          $query .= " ORDER BY absenpameran.created_absen DESC";
          break;
      }
      ...
      // Bind parameters untuk filter tanggal jika ada
      if (!empty($_GET['date_filter'])) {
      ...
      if (mysqli_num_rows($result) > 0) {
        ...
        echo '<th class="text-left">Kode Absen</th>';
        ...
        while ($row = mysqli_fetch_assoc($result)) {
          ...
          echo '<td>' . $row['kode_absen'] . '</td>'; // Menampilkan kode_absen dari tabel absenpameran
        ...
        $count_query = "SELECT COUNT(*) AS total FROM absenpameran";
        $count_result = mysqli_query($koneksi, $count_query);
        $total_data = mysqli_fetch_assoc($count_result)['total'];

        // Tampilkan total kehadiran
        echo '<div class="mt-4">';
        echo '<strong>Total Kehadiran: ' . $total_data . '</strong>';
        echo '</div>';
