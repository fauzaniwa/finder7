...
// Query untuk mengambil data absensi seminar terbaru dengan nama user
$query_absen = "
    SELECT 
        a.id_absen, 
        a.kode_absen, 
    ...
    FROM 
        absen a
    JOIN 
        tiket t ON a.kode_absen = t.tiket_code
    WHERE 
        t.nama_lengkap LIKE ? OR t.email LIKE ? OR t.no_whatsapp LIKE ?
    ORDER BY 
        a.created_absen DESC
    LIMIT ?, ?
";
...
mysqli_stmt_bind_param($stmt_absen, "sssii", $search_param, $search_param, $search_param, $offset, $limit);
...
<head>
    <meta charset="UTF-8">
    <title>Daftar Hadir Seminar</title>
...
<body class="bg-dark text-white font-poppins flex">
    ...
    <main class="flex-grow p-6 overflow-x-hidden">
        ...
        <header class="bg-dark-card p-4 flex justify-between items-center lg:hidden sticky top-0 z-40">
            ...
            <span class="text-lg font-semibold text-light-gray">Daftar Hadir Seminar</span>
            <div class="w-6 h-6"></div>
        </header>

        <div class="bg-dark-card p-8 rounded-xl shadow-lg mt-4 lg:mt-0">
            <h1 class="text-3xl font-bold mb-4 text-primary-green">Daftar Hadir Seminar & Workshop</h1>
            <p class="text-mid-gray mb-6">Berikut adalah 50 data kehadiran seminar terbaru. Total: <?php echo $total_data; ?></p>
            ...
            <div class="flex flex-col sm:flex-row justify-between items-center mb-6 space-y-4 sm:space-y-0">
                ...
                <div class="flex items-center w-full sm:w-auto space-x-2">
                    <form action="" method="GET" class="flex items-center w-full">
                        ...
                        <input type="text" name="q" placeholder="Cari nama atau email..." value="<?php echo htmlspecialchars($search_query); ?>" class="w-full sm:w-auto px-4 py-2 rounded-md bg-dark-gray text-light-gray focus:outline-none focus:ring-2 focus:ring-primary-green">
                ...
                <table class="min-w-full divide-y divide-gray-700">
                    <thead class="bg-dark-gray">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-light-gray uppercase tracking-wider">No. WhatsApp</th>
                        ...
                        <?php if (!empty($data_absensi)): ?>
                            ...
                            <?php foreach ($data_absensi as $absen): ?>
                                <tr class="bg-dark-card hover:bg-dark-gray transition-colors">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-mid-gray"><?php echo htmlspecialchars($absen['nama_lengkap']); ?></td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-mid-gray"><?php echo htmlspecialchars($absen['email']); ?></td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-mid-gray"><?php echo htmlspecialchars($absen['no_whatsapp']); ?></td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-mid-gray"><?php echo htmlspecialchars($absen['created_absen']); ?></td>
                                </tr>
                            <?php endforeach; ?>
                        <?php else: ?>
                            <tr>
                                <td colspan="5" class="px-6 py-4 text-center text-sm text-mid-gray">Tidak ada data absensi seminar.</td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
