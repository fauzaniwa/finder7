...
<body>
    ...
    <section id="jadwal" class="container flex flex-col max-w-full bg-[#0D0D0D] gap-4 py-10 pt-28">
        <!-- H1 -->
        <h1 style="font-family: 'Work Sans'" class="text-white px-6 md:px-16 text-2xl md:text-3xl md:mx-auto">Jadwal
            Acara
        </h1>

        <!-- Filter Jadwal -->
        <div class="flex flex-row gap-6 px-6 md:px-16 md:mx-auto">
            <!-- Semua Jadwal -->
            ...
            foreach ($unique_jadwal_events as $jadwal_event) {
                ...
                echo '<a href="?filter=' . urlencode($jadwal_event) . '#jadwal" class="filter-button py-2 px-4 text-white rounded-full hover:bg-white hover:bg-opacity-25 text-lg ' . $activeClass . '">' . htmlspecialchars($formatted_date) . '</a>';
            }
            ?>
        ...
        foreach ($events_data as $event):
            ...
            <div
                ...
                <!-- Flex-Kiri -->
                <div class="flex flex-col lg:flex-row gap-4 lg:gap-16 lg:items-center">
                    ...
                    <?php if ($event['event_status'] == 1): // Pengecekan status event ?>
                        <form method="post">
                            ...
                            <button style="font-family: 'Work Sans'"
                                class="get-ticket-button w-[200px] h-fit border-[1px] hover:bg-white hover:bg-opacity-25 py-2 px-6 border-white text-white rounded-full md:text-lg">Dapatkan
                                Tiket</button>
                        </form>
                    <?php elseif ($event['event_status'] == 2): // Status event belum dimulai ?>
                        <button style="font-family: 'Work Sans'"
                            class="get-ticket-button w-[200px] h-fit border-[1px] hover:bg-white hover:bg-opacity-25 py-2 px-6 border-white text-white rounded-full md:text-lg"
                            disabled>Pendaftaran Belum Dibuka</button>
                    <?php else: ?>
                        <button style="font-family: 'Work Sans'"
                            class="get-ticket-button w-[200px] h-fit border-[1px] hover:bg-white hover:bg-opacity-25 py-2 px-6 border-white text-white rounded-full md:text-lg"
                            disabled>Event Sudah Berakhir</button>
                    <?php endif; ?>
                <?php else: ?>
                    <div>
